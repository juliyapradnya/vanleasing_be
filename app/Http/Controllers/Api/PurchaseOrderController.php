...
use App\SalesOrder;
use App\VehicleSold;
use App\BaseInterest;
use App\BaseInterestDetail;
...
class PurchaseOrderController extends Controller
{
    ...
    //
     ...
     public function showVehicleNumberExceptSold(){
        $purchaseorder = DB::table('purchase_orders')
...
//show contract by id in vehicle performance
public function showContractById($id){
    $purchaseorder = DB::table('purchase_orders')
                    ->join('sales_orders','sales_orders.id_purchase_order','=','purchase_orders.id')
                    ->join('rehiring_orders','rehiring_orders.id_purchase_order','=','rehiring_orders.id_purchase_order')
                     ->select('purchase_orders.vehicle_registration','sales_orders.cust_name','sales_orders.next_step_status_sales','sales_orders.total_income','sales_orders.margin_term'
                     ,'sales_orders.annual_mileage','rehiring_orders.vehicle_return_date')
                     ->whereRaw('purchase_orders.id = '.$id)
                     ->get();
...
//show vehicle by id in vehicle performance
public function showVehicleById($id){
    $purchaseorder = DB::table('purchase_orders')
                    ->join('sales_orders','sales_orders.id_purchase_order','=','purchase_orders.id')
                     ->select('purchase_orders.vehicle_registration','purchase_orders.vehicle_manufactur','purchase_orders.vehicle_model','purchase_orders.vehicle_variant','purchase_orders.colour'
                     ,'purchase_orders.service_schedule_miles','purchase_orders.last_service_mileage'
                     ,'purchase_orders.mot_due_date','purchase_orders.rfl_due_date')
                     ->selectRaw('round(SUM(annual_mileage),2) as current_mileage')
                     ->whereRaw('purchase_orders.id = '.$id)
                     ->first();
...
//show cost and funding by id in vehicle performance
public function showCostandFunding($id){
    $purchaseorder = DB::table('purchase_orders')
                    ->leftJoin('other_costs','other_costs.id_purchase_order','=','purchase_orders.id')
                    ->leftJoin('rehiring_orders','rehiring_orders.id_purchase_order','=','rehiring_orders.id_purchase_order')
                     ->select('purchase_orders.vehicle_registration','purchase_orders.price_otr','purchase_orders.hp_interest_per_annum','other_costs.amount_oc',
                     'purchase_orders.vehicle_manufactur','purchase_orders.vehicle_model','purchase_orders.vehicle_variant','purchase_orders.colour','rehiring_orders.vehicle_return_date')
                     ->selectRaw('round(SUM(amount_oc),2) as other_cost')
                     ->whereRaw('purchase_orders.id = '.$id)
                     ->first();
...
//dashboard
public function sumTotalIncome(){
    $purchaseorder = DB::table('sales_orders')
                ->join('purchase_orders','purchase_orders.id','=','sales_orders.id_purchase_order')
                ->selectRaw('round(SUM(total_income),2) as sum_total_income, purchase_orders.vehicle_registration')
                ->first();
...
//dashboard
public function sumRentalIncome(){
    $purchaseorder = DB::table('sales_orders')
                    ->join('purchase_orders','purchase_orders.id','=','sales_orders.id_purchase_order')
                    ->selectRaw('round(SUM(rental_income),2) as sum_rental_income')
                    ->first();
...
//dashboard
public function sumOtherIncome(){
    $purchaseorder = DB::table('other_incomes')
                    ->join('purchase_orders','purchase_orders.id','=','other_incomes.id_purchase_order')
                    ->selectRaw('round(SUM(amount_oi),2) as sum_other_income')
                    ->first();
...
//dashboard
public function sumResidualValue(){
    $purchaseorder = DB::table('purchase_orders')
                    //->join('purchase_orders','purchase_orders.id','=','sales_orders.id_purchase_order')
                    ->selectRaw('round(SUM(residual_value),2) as sum_residual_value')
                    ->first();
...
//show data dashboard 
//ini yang isi tanggal
//kalau tidak isi hapus saya date 1 date 2
//belum bisa dipakai
...
public function availableStock(Request $request){
    ...
    $purchaseorder = DB::table('purchase_orders')
                    ->select('purchase_orders.vehicle_registration','purchase_orders.vehicle_manufactur','purchase_orders.vehicle_model','purchase_orders.vehicle_variant','purchase_orders.status_next_step','purchase_orders.tgl_available')
    ...
    public function compilationDB(Request $request){
    ...
    public function store(Request $request){
        ...
        $validate = Validator::make($storeData, [
            ...
            'service_maintenance'           => 'required',
            'mot_due_date'                  => 'required|date_format:Y-m-d',
            'rfl_due_date'                  => 'required|date_format:Y-m-d',
            'service_schedule_miles'        => 'required',
            'service_schedule_years'        => 'required',
            'last_service_mileage'          => 'required',
            'last_service_date'             => 'required|date_format:Y-m-d',
        ...
        $baseInterestDetail = BaseInterestDetail::create([
            ...
            'total_base_interest' => (($baseInterest->percentage/100)/12) * $purchaseorder->financing_amount
        ]);
